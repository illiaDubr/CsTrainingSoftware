...
import { AdminMatchItem } from '../../../src/types';
import { showAlert, showConfirm } from '../../../src/utils/alert';
import { colors, radius, shadows } from '../../../src/theme';

const CLASS_LABEL: Record<string, string> = { esea: 'ESEA', other: 'Другое' };
...
export default function AdminMatchesScreen() {
  ...
  const load = async () => {
    try {
      setItems(await adminService.getMatches());
    } catch {
      // тихо
    } finally {
      setLoading(false);
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((m) => `${m.opponent} ${m.group_name} ${m.created_by_username}`.toLowerCase().includes(q));
  }, [items, query]);

  const handleDelete = (item: AdminMatchItem) => {
    showConfirm('Удалить матч?', `vs ${item.opponent}`, async () => {
      setDeletingId(item.id);
      try {
        await adminService.deleteMatch(item.id);
  ...
  return (
    ...
    >
      ...
      <TextInput
        style={styles.search}
        placeholder="Поиск по сопернику, группе..."
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((m) => (
            <View key={m.id} style={styles.row}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.rowTitle} numberOfLines={1}>vs {m.opponent}</Text>
                <Text style={styles.rowSub} numberOfLines={1}>
                  {CLASS_LABEL[m.match_class] ?? m.match_class} · {new Date(m.scheduled_at).toLocaleString('ru-RU')} · {m.group_name}
                </Text>
              </View>
              <TouchableOpacity
