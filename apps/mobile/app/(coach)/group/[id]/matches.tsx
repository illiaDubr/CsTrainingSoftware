...
export default function CoachGroupMatchesScreen() {
  ...
  const canModify = (m: Match) => m.created_by === user?.id || user?.role === 'coach';
  ...
  return (
    <View style={styles.wrapper}>
        ...
        {dayMatches.length === 0 ? (
          ...
          dayMatches.map((m) => (
            <MatchCard
              ...
              match={m}
              showAuthor
              onEdit={canModify(m) ? () => router.push(`/(coach)/edit-match?matchId=${m.id}`) : undefined}
      ...
      <FAB onPress={() => router.push(`/(coach)/create-match?groupId=${id}&date=${selectedDate}`)} />
    </View>
  );
}
