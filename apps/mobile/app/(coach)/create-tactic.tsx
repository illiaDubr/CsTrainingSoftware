...
import { Nade, NadeMapSummary, NadeSide, TacticArrow } from '../../src/types';
...
export default function CreateTacticScreen() {
  ...
  const handleCreate = async () => {
    ...
    setLoading(true);
    try {
      await tacticsService.createTactic({
      ...
      showAlert('Ошибка', 'Не удалось создать тактику');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={styles.inner} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.title}>Новая тактика</Text>

        <TextInput
          style={styles.input}
          placeholder="Название раунда (например: A-execute, дефолт CT)"
        ...
        <TextInput
          ...
          placeholder="Описание раунда: план, роли, тайминги... (необязательно)"
          placeholderTextColor="#5B677D"
          value={description}
        ...
        <Text style={styles.label}>Карта (из наших раскидок)</Text>
        {mapsLoading ? (
          <ActivityIndicator color="#f59e0b" style={{ marginBottom: 16 }} />
        ) : availableMaps.length === 0 ? (
          <Text style={styles.hintText}>Сначала загрузи хотя бы одну раскидку — тогда появится выбор карты</Text>
        ) : (
          <View style={styles.row}>
            {availableMaps.map(m => (
              <TouchableOpacity
                ...
                {arrows.map((a, i) => (
                  <View key={i} style={styles.arrowRow}>
                    ...
                    <TextInput
                      ...
                      editable={!loading}
                    />
                    <TouchableOpacity onPress={() => removeArrow(i)} disabled={loading} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
